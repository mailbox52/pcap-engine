...
- **Phase 4c is done and confirmed in a browser and on the deployed Vercel site** (a `TS2352` type error was hit and fixed along the way: `PcapIndex.filter()` returns `Uint32Array`, not `number[]` — `src/lib/parse-file.ts`'s `runFilter` no longer casts it). Screenshot: `port 443` on a 127-packet capture returned 36 real matches (SYN/SYN-ACK/ACK/FIN across several TCP conversations).
- **The original `sample.pcap` / `sample.pcapng` were flawed and have been replaced.** The first version (used through Phase 4b) put random bytes directly after the IP header with no real TCP/UDP/ARP structure, so every packet was IPv4 with garbage "ports" — filters like `port 443` or `arp` looked broken (they weren't; the data just had no real headers to match). The new 127-packet `sample.pcap`/`sample.pcapng` have real TCP handshakes, DNS queries/replies, plain UDP, ICMP pings, and ARP request/reply pairs. If a future session needs test data with actual meaning, regenerate from realistic headers, not random payload bytes.
- **Phase 4d is done:** `public/sample.pcap` (9 KB, the new realistic capture) plus a "Try a sample capture" button in `PcapUploader.tsx` (`loadSample()`, a plain `fetch("/sample.pcap")` into the same `openFile()` path as a real upload). Confirmed: the static file is served correctly by `next start` (`200`, `application/vnd.tcpdump.pcap`), and the full fetch → parse → summary → filter path was run end to end against the real CI-built Wasm (127 packets, `arp` → 16 matches, `port 443` → 36 matches — matching the browser screenshot). `npm run verify` is now 55 checks, including 4 that read `public/sample.pcap` directly and would fail if it were ever removed or replaced with something unrealistic again.
- **Phase 4e was already satisfied** — `src/app/page.tsx` has always had "Your file never leaves this device," which is Phase 2's original wording for the spec's suggested line.
- **All of Phase 4 (4a-4e) is now done.** Only 4c needed a CI Wasm rebuild + confirmation; 4d needed no Rust changes, so it does not need a CI round trip.
- **Next:** nothing planned. Ideas for a future phase, if wanted: TCP stream reassembly, a hex/ASCII view for the full packet (currently capped at the first 32 bytes), export of the filtered view as CSV, or a traffic-over-time chart. None of these are committed to — check with the user before starting one.
...
**4d. Sample capture** (done, confirmed against the real Wasm)
- [x] A "Try a sample" button loads a small bundled `.pcap` (`public/sample.pcap`, 9 KB, well under the 200 KB budget), fetched as a static file (`fetch("/sample.pcap")`) and fed through the exact same `openFile()` path as a user upload — parse, batches, summary, filters, detail all work identically.
- [x] The sample includes a mix of TCP (8 realistic handshakes with SYN/SYN-ACK/ACK/data/FIN), DNS queries and replies, plain UDP, ICMP echo request/reply pairs, and ARP request/reply pairs — 127 packets total. The original `sample.pcap`/`sample.pcapng` handed over earlier were regenerated with real protocol headers for the same reason (see Status): the first version used random bytes as payload, which made `port 443`-style filters meaningless.

**4e. Privacy note**
- [x] Already on the page (`src/app/page.tsx`): "Your file never leaves this device." — same guarantee as the spec's suggested wording.
